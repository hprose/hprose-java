...
final class IntArrayUnserializer implements Unserializer {

    public final static IntArrayUnserializer instance = new IntArrayUnserializer();

    final static int[] read(Reader reader, ByteBuffer buffer) throws IOException {
        int tag = buffer.get();
        switch (tag) {
    ...
    final static int[] read(Reader reader, InputStream stream) throws IOException {
        int tag = stream.read();
        switch (tag) {
            case TagNull: return null;
            case TagList: {
